...
syd () {
  ffmpeg -rtsp_transport tcp -stimeout 50000000 -i "rtsp://192.168.1.10:554/user=admin&password=&channel=0&stream=0.sdp?real_stream" -c copy -an -f segment -strftime 1 -reset_timestamps 1 -segment_time 900 -segment_atclocktime 1 -segment_format mp4 "syd__%Y-%m-%d__%H-%M-%S.mp4"
}
...
#  if test $(find "$lastfile" -mmin +3) ; then
#            echo "File is old"


            ffmpeg -loglevel panic -rtsp_transport tcp -stimeout 50000000 -i "rtsp://192.168.1.10:554/user=admin&password=&channel=0&stream=0.sdp?real_stream" -c copy -map 0 -an -f segment -strftime 1 -reset_timestamps 1 -segment_time 900 -segment_atclocktime 1 -segment_format mp4 "syd__%Y-%m-%d__%H-%M-%S.mp4" < /dev/null &
